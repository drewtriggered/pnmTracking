...
Push needs four things in place, and it is silent about whichever one is
missing — so set them in this order and check each:

1. **`VITE_FIREBASE_VAPID_KEY` in `.env`, then a hosting deploy.** The key is
   baked into the bundle at build time, so setting it means
   `npm run deploy:hosting` before any device can register.
2. **`npm run deploy:functions`.** Nothing sends without it, and it needs the
   Blaze plan (step 2 above).
3. **The App URL on the Settings tab**, starting with `https://`. FCM rejects
   a link that isn't absolute https, so the screen warns about one it would
   reject; the send drops the bad link rather than failing with it.
4. **A registered device.** *Turn on reminders* on the Settings tab, once per
   device per brother. On iPhone the app has to be installed to the home
   screen first (Share → Add to Home Screen, iOS 16.4+) — iOS delivers web
   push to nothing else.

Then hit *Send me a test*. It reports how many devices FCM accepted, and fails
loudly with FCM's own reason if that is none.

With the app **open**, a reminder appears as a banner at the top of the page
rather than a system notification: FCM hands a push straight to a visible page
and draws nothing itself. Close or background the app to see the real thing.
...
A brother who has tokens but reached no device is logged as a warning with
FCM's error codes, which is what a device that has been reinstalled, or a
notification permission revoked in the browser, looks like from the server.

### How push is wired

Two service workers, deliberately at different scopes:

- `sw.js` (generated by vite-plugin-pwa) at `/` — the offline app shell.
- `firebase-messaging-sw.js` at `/firebase-cloud-messaging-push-scope` — the
  background push handler, registered by `src/lib/push.ts`.

A registration is keyed by its scope, so both at `/` means each one replaces
the other and pushes end up at a worker with no handler. The messaging worker
is served `no-cache` and kept out of the PWA precache for the same reason: a
stale copy of it is a copy that stops showing notifications.

The Firebase SDK draws the notification itself for any message carrying a
`notification` block, so `onBackgroundMessage` only handles data-only pushes —
showing one there too is what puts every reminder on the lock screen twice.
